...
void delayMicroseconds(unsigned int us)
{
	...
	uint64_t cmp_value = (uint64_t) Chip_Clock_GetSystemClockRate() * us / 1000000;
	// disable RIT – compare value may only be changed when RIT is disabled
	Chip_RIT_Disable(LPC_RITIMER);
	// set compare value to RIT
	LPC_RITIMER->COMPVAL = (uint32_t) cmp_value;
	LPC_RITIMER->COMPVAL_H = (uint32_t) (cmp_value >> 32);
	// clear RIT counter (so that counting starts from zero)
	LPC_RITIMER->COUNTER = (uint32_t) 0;
	LPC_RITIMER->COUNTER_H = (uint32_t) 0;
	// enable RIT
	Chip_RIT_SetCTRL(LPC_RITIMER, (1 << 3));
